...
ResultSet theResult=theStatement.executeQuery("select * from login.convertrtf where fsize > 0");
...
%>
  ...
  val = St.executeUpdate("update filemonitor set dest='ENTERING APPLICATION SERVER 2' where state=1");
      for(int i=1;i<5;i++)
      {
...
St.executeUpdate("delete from login.convertrtf where username='"+pp+"'"); 

val = St.executeUpdate("update filemonitor set source='EXITING APPLICATION SERVER 2' where state=1");
 ...
 while(theResult3.next())         
         {
     count=theResult3.getInt(2);
     }
     count=count+1;
     int val2 = St.executeUpdate("update login.number set rtf='"+count+"'");
 ...
 while(theResult3.next())         
         {
     count=theResult3.getInt(4);
     }
     count=count+1;
     val2 = St.executeUpdate("update login.inout set rtfout='"+count+"'");
...
if(temp==0)
{
 ...
 while(theResult.next())         
         { 
     int idle1=theResult.getInt(5);
     idle1= idle1+5;
     int val = St.executeUpdate("update login.number set idlertf='"+idle1+"'");
