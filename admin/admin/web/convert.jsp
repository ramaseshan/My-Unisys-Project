...
ResultSet theResult=theStatement.executeQuery("select * from login.convert where fsize > 0");
...
%>
  ...
  val = St.executeUpdate("update filemonitor set dest='ENTERING APPLICATION SERVER 1' where state=1");
      for(int i=1;i<5;i++)
      {
...
St.executeUpdate("delete from login.convert where username='"+pp+"'"); 

val = St.executeUpdate("update filemonitor set source='EXITING APPLICATION SERVER 1' where state=1");
 ...
 while(theResult3.next())         
         {
     count=theResult3.getInt(1);
     }
     count=count+1;
     int val2 = St.executeUpdate("update login.number set txt='"+count+"'");
...
count=0;
 ...
 while(theResult3.next())         
     ...
     count=count+1;
     val2 = St.executeUpdate("update login.inout set txtout='"+count+"'");
...
if(temp==0)
{
 ...
 while(theResult.next())         
         { 
     int idle1=theResult.getInt(4);
     idle1=idle1+5;
     int val = St.executeUpdate("update login.number set idletxt='"+idle1+"'");
